...
def test_import_sales_triggers_pipeline_automatically_and_updates_summary() -> None:
    ...
    response = client.post(
        ...
        headers={"Authorization": f"Bearer {token}", "X-Correlation-ID": "pipe-auto-1"},
    )
    ...
    summary_response = client.get(
        "/v1/summary?company_id=cmp_acme&period_ref=2026-07",
        headers={"Authorization": f"Bearer {token}"},
    )
