...
def _payload(company_id: str) -> dict:
    return {
        ...
        "contacts": [{"channel_type": "email", "value": "financeiro@acme.com"}],
        "external_refs": [{"source_system": "omie", "external_id": "CUST-800"}],
        "source_system": "omie",
...
def test_tenant_spoofing_is_blocked() -> None:
...
def test_create_all_not_called_in_request_path(monkeypatch) -> None:
    from app.shared.infrastructure.db.base import Base
...
def test_integrity_error_is_mapped_to_conflict(monkeypatch) -> None:
    import app.modules.business.interfaces.api.routes as business_routes

    @dataclass
    class _UpsertStub:
        def execute(self, command: UpsertCustomerCommand) -> CustomerAggregate:
            raise IntegrityError("insert", {}, Exception("duplicate"))

    @dataclass
    class _ContainerStub:
        upsert_customer: _UpsertStub

    monkeypatch.setattr(business_routes, "build_customer_container", lambda db: _ContainerStub(upsert_customer=_UpsertStub()))
    ...
    assert response.json()["detail"] == "customer conflict detected"
    app.dependency_overrides.clear()
