        ...
        *,
        ...
        for item in records:
            try:
                ...
                refs = (
                    ExternalReference(source_system=source_system, external_id=str(item.get("external_id") or "unknown")),
                )
                self.upsert_customer.execute(
                    UpsertCustomerCommand(
                        ...
                        document_number=(str(item.get("document") or "").strip() or None),
                        status=CustomerStatus(str(item.get("status") or "active").lower()),
                        billing_address=None,
                        contacts=tuple(contacts),
                        external_refs=refs,
                        source_system=source_system,
                        source_record_id=str(item.get("source_record_id") or item.get("external_id") or "unknown"),
                        canonical_schema_version="2.0.0",
                        correlation_id=correlation_id,
                    )
                )
            ...
            try:
                ...
                self.upsert_product.execute(
                    UpsertProductCommand(
                        company_id=company_id,
                        sku=str(item.get("sku") or item.get("external_id") or "sku-unknown"),
